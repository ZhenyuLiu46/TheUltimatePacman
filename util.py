...
def findModule(function_or_class, module_name_postfix):
    ...
    for moduleDir in pythonPathDirs:
        ...
        moduleNames = [f for f in os.listdir(moduleDir) if f.endswith(module_name_postfix)]
        for modulename in moduleNames:
            ...
            try:
                ...
                continue
            if function_or_class in dir(module):
                return getattr(module, function_or_class)
    return None

def findStudentModule(module_name_postfix):
    moduleNames = [f for f in os.listdir(".") if f.endswith(module_name_postfix)]
    if len(moduleNames) == 1:
        return moduleNames[0]
    elif  len(moduleNames) > 1:
        print "Warning: more than one ", module_name_postfix, " file found!"
        print "Using:", moduleNames[0]
        return moduleNames[0]
    else:
        return None
