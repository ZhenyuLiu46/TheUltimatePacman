...
if __name__ == '__main__':
    ...
    moduleDict = {}
    for cp in codePaths:
        module = util.findStudentModule(cp)
        moduleName = re.match('.*?([^/]*)\.py', module).group(1)
        moduleKey = re.match('.*?([^/]*)\.py', cp).group(1)
        moduleDict[moduleKey] = loadModuleFile(moduleName, os.path.join(options.codeRoot, module))
